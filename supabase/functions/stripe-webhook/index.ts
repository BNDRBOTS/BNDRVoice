// ══════════════════════════════════════════════════════════════
// BNDR VoiceEngine — Stripe Webhook Edge Function
//
// Deploy:
...
if (!stripeSecret || !webhookSecret || !serviceRoleKey || !supabaseUrl) {
  throw new Error('Missing required Stripe/Supabase function secrets')
}
...
async function upsertSubscription(
  ...
  if (!(await isKnownUser(userId))) {
    console.warn(`Subscription ${sub.id} has an invalid or unknown user mapping; skipping`)
    return
  }
...
async function isKnownUser(userId: string): Promise<boolean> {
  ...
  if (error) {
    ...
    throw new Error(`Could not validate subscription owner: ${error.message}`)
  }
